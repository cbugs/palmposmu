...
patch(ActionpadWidget.prototype, {
    ...
    async submitOrder() {
        ...
        if (!this.uiState.clicked) {
            ...
            try {
                await self.orm.call("pos.order", "check_order_status", ["", this.pos.selectedOrder.pos_reference]).then(function(result){
                    if (result == false){
    ...
    async processOrderForKitchen() {
        var self = this;
        const orderData = {
            'pos_reference': this.pos.selectedOrder.pos_reference,
            'config_id': this.pos.selectedOrder.config_id.id,
            'table_id': this.pos.selectedOrder.table_id.id,
            'session_id': this.pos.selectedOrder.session_id.id
        };
    ...
    hasQuantity(order) {
        if (!order) {
            ...
            return (
                order.lines.reduce((totalQty, line) => totalQty + (line.qty || 0), 0) > 0
            );
        }
    },
